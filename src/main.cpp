...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < fctx->nb_streams; ++i) {
		avcodec_params = fctx->streams[i]->codecpar;
		codec = avcodec_find_decoder(avcodec_params->codec_id);
		if (codec == nullptr)
			continue;
		bool is_video = avcodec_params->codec_type == AVMEDIA_TYPE_VIDEO;
		string ct =  is_video ? "video" : "not video";
		cout << "Codec(" << ct << "): " << codec->long_name << endl;
		if (!is_video)
			continue;
		...
		double dur;
		double time_base = av_q2d(fctx->streams[i]->time_base);
